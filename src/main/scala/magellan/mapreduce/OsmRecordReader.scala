...
private[magellan] class OsmRecordReader extends RecordReader[Tuple2[String, String], OsmShape] {
  ...
  override def getCurrentKey() : Tuple2[String, String] = {
    val current = nodes.head
    (current.label, (current \ "@id").text)
  }
  ...
  def getOsmNode(shape: Node) = {
    new OsmNode((shape \ "@id").text, (shape \ "@lat").text.toFloat, (shape \ "@lon").text.toFloat, getTags(shape))
  }
  ...
  def getOsmRelation(shape: Node) = {
    new OsmRelation((shape \ "@id").text, (shape \ "member").map(r => (r \ "@ref").text), getTags(shape))
  }
