...
class PolyLineSuite extends FunSuite with TestSparkContext {
  ...
  test("bounding box") {
    ...
    val BoundingBox(xmin, ymin, xmax, ymax) = polyline.getboundingBox
    assert(xmin === 0.0)
    assert(ymin === 0.5)
    assert(xmax === 1.0)
    assert(ymax === 1.0)
  ...
  test("bounding box of multiLine") {
    val polyline = PolyLine(Array(0,2),Array(Point(1.0, 1.0), Point(2.0, 2.0), Point(3.0, 3.0), Point(4.0, 4.0)))
    val BoundingBox(xmin, ymin, xmax, ymax) = polyline.getboundingBox
    assert(xmin === 1.0)
    assert(ymin === 1.0)
    assert(xmax === 4.0)
